...
const { assertRevert } = require('./helpers/assertThrowBuidlerEvm')
...
const { getInstalledApp } = require('@aragon/contract-helpers-test/src/aragon-os/apps')
...
contract('DisputableDelay', ([rootAccount, otherAccount]) => {
  ...
  before('deploy base apps', async () => {
    ...
    agreement = await agreementDeployer.deployAndInitializeAgreementWrapper({ rootAccount })
    ...
    CHALLENGE_ROLE = await agreementDeployer.base.CHALLENGE_ROLE()
    DELAY_EXECUTION_ROLE = await delayBase.DELAY_EXECUTION_ROLE()
  })

  beforeEach('deploy dao and delay', async () => {
    const appId = nameHash('delay.aragonpm.eth')
    const newDelayAppReceipt =
      await agreementDeployer.dao.newAppInstance(appId, delayBase.address, '0x', false, { from: rootAccount })
    delay = await DisputableDelay.at(await getInstalledApp(newDelayAppReceipt, appId))

    await agreementDeployer.acl.createPermission(agreement.address, delay.address, SET_AGREEMENT_ROLE, rootAccount)
    await agreementDeployer.acl.createPermission(ANY_ADDR, delay.address, CHALLENGE_ROLE, rootAccount)
    await agreementDeployer.acl.createPermission(rootAccount, delay.address, DELAY_EXECUTION_ROLE, rootAccount)
  })
  ...
  describe('initialize(uint256 _executionDelay)', () => {
    ...
    describe('agreement.activate()', () => {
      ...
      describe('setExecutionDelay(uint256 _executionDelay)', () => {
        it('sets the execution delay correctly', async () => {
          await agreementDeployer.acl.createPermission(rootAccount, delay.address, SET_DELAY_ROLE, rootAccount)
      ...
      describe('canForward(address _sender, bytes _evmScript)', () => {
      ...
      describe('submit a delayed execution script', () => {
        ...
        const itUpdatesExecutionStateCorrectly = () => {
          ...
          it('stores delayed execution script and updates new script index', async () => {
            ...
            assert.equal(actualEvmScript, script)
          ...
          describe('canClose(uint256 _delayedScriptId)', () => {
            ...
            it('returns true when script is cancelled', async () => {
              await delay.cancelExecution(delayedScriptId)
              const canClose = await delay.canClose(delayedScriptId)
              assert.isTrue(canClose)
            })

            it('returns true when script is executed', async () => {
              await delay.mockIncreaseTime(DELAY_LENGTH)
          ...
          describe('execute(uint256 _delayedScriptId)', () => {
            ...
            it('reverts when evmScript reenters delay contract, attempting to execute same script twice', async () => {
              ...
              const delayReceipt = await delay.delayExecution(reenteringScript, '0x')
            ...
            it('reverts when evmScript calls function on Agreements contract', async () => {
              ...
              const delayReceipt = await delay.delayExecution(agreementScript, '0x')
          ...
          describe('cancelExecution(uint256 _delayedScriptId)', () => {
            ...
            it('cancels execution when account with permission attempts to cancel', async () => {
              ...
              await agreementDeployer.acl.createPermission(otherAccount, delay.address, cancelExecutionRole, rootAccount)
        ...
        describe('delayExecution(bytes _evmScript, bytes _context)', () => {
          beforeEach(async () => {
            delayCreatedTimestamp = await delay.getTimestampPublic()
            const delayExecutionReceipt = await delay.delayExecution(script, '0x')
          ...
          it('reverts when permission revoked', async () => {
            await agreementDeployer.acl.revokePermission(rootAccount, delay.address, DELAY_EXECUTION_ROLE)
            await assertRevert(delay.delayExecution(script, '0x'), 'APP_AUTH_FAILED')
          })

          itUpdatesExecutionStateCorrectly()
        })

        describe('forward(bytes _evmScript, bytes _context)', () => {
          beforeEach('create delayed script', async () => {
            delayCreatedTimestamp = await delay.getTimestampPublic()
            const delayExecutionReceipt = await delay.forward(script, '0x')
            delayedScriptId = getEventArgument(delayExecutionReceipt, 'DelayedScriptStored', 'delayedScriptId')
            actionId = getEventArgument(delayExecutionReceipt, 'DelayedScriptStored', 'actionId')
          })
          ...
          it('reverts when permission revoked', async () => {
            await agreementDeployer.acl.revokePermission(rootAccount, delay.address, DELAY_EXECUTION_ROLE)
            await assertRevert(delay.forward(script, '0x'), 'DELAY_CANNOT_FORWARD')
          })
    ...
    describe('app not activated in agreements', () => {
      ...
      describe('canForward(address _sender, bytes _evmScript)', () => {
        it('returns false when not activated in agreements', async () => {
          assert.isFalse(await delay.canForward(rootAccount, '0x'))
        })
      })

      describe('forward(bytes _evmScript, bytes _context)', () => {
        it('reverts when not activated in agreements', async () => {
          await assertRevert(delay.forward(script, '0x'), "DELAY_CANNOT_FORWARD")
        })
      })

      describe('delayExecution(bytes _evmScript, bytes _context)', async () => {
        it('reverts when not activated in agreements', async () => {
          await assertRevert(delay.delayExecution(script, '0x'), "DISPUTABLE_AGREEMENT_STATE_INVAL")
        })
      })
    })
